...
type AdminTab = "users" | "recruiters" | "jobs" | "communities" | "reports";
    ...
    <div className="min-h-screen bg-purple-soft">
      <div className="p-4 sm:p-6 max-w-7xl mx-auto">
        ...
        <Card className="bg-white shadow-md">
          ...
          >
            <div className="px-6">
              <TabsList className="grid grid-cols-5 mb-6 bg-purple-100/50">
...
function UsersTab({ searchQuery }: { searchQuery: string }) {
  ...
  // For a real implementation, use this query instead of mockUsers
  /*
  ...
  */
  ...
  // For demo purposes, simulate loading state and use mock data
  ...
  // Toggle user recruiter status (mock implementation)
  const toggleRecruiterMutation = useMutation({
    mutationFn: async ({ userId, makeRecruiter }: { userId: number, makeRecruiter: boolean }) => {
      ...
      console.log(`Setting user ${userId} recruiter status to ${makeRecruiter}`);
      return { success: true };
    },
    onSuccess: () => {
      toast({
...
function RecruitersTab({ searchQuery }: { searchQuery: string }) {
  ...
  const isLoading = false;
  const recruiters = mockRecruiters;
              ...
              {filteredRecruiters.length > 0 ? (
                filteredRecruiters.map((recruiter) => (
                  <tr key={recruiter.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-center">{recruiter.jobsCount}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center space-x-2">
                        <Button variant="ghost" size="sm">
                          View Jobs
  ...
  const mockJobs = [
    {
      ...
      createdAt: "2025-05-13T09:51:15.873Z",
      user: {
        id: 7,
        name: "Ananya Singh",
        username: "ananyasingh",
          ...
          <CardContent>
            <div className="h-64 flex items-center justify-center border rounded p-4">
              <div className="text-center text-gray-500">
                ...
                <p>Usage metrics will be displayed here</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="h-64 flex items-center justify-center border rounded p-4">
              <div className="text-center text-gray-500">
                ...
                <p>Industry distribution will be displayed here</p>
              </div>
            </div>
          </CardContent>
        </Card>
